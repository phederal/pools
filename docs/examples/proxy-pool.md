# Proxy Pool Example

Advanced example demonstrating proxy pool management.

## Source Code

[View on GitHub](https://github.com/yourusername/pools/blob/main/examples/proxy-pool.ts)

## Running

```bash
bun run examples/proxy-pool.ts
```

## Features Demonstrated

- CRUD operations with metadata
- Event handling and auto-tracking
- Complex queries with multiple filters
- Pool combination and deduplication
- Transformations (partition, groupBy, sample)
- Weighted selectors
- PoolBinder for resource allocation

See the [source code](https://github.com/yourusername/pools/blob/main/examples/proxy-pool.ts) for the complete implementation.
